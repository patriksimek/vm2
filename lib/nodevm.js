...
class NodeVM extends VM {
	...
	constructor(options = {}) {
		...
		// `require` config, reaches host `fs` or host-context externals —
		// any capability the outer config denied).
		//
		...
		//    `Resolver` instance), not just "truthy" or "typeof object".
		//    `makeResolverFromLegacyOptions` destructures every non-config value
		//    to all-`undefined`, producing the same NESTING_OVERRIDE-only
		//    resolver. The shapes that collapse to the insecure resolver:
		...
		//      - `require: []` / `new Array()` / `new Date()` / `/x/` /
		//        `new Map()` / `new String()` (array, exotic instance, boxed
		//        primitive) → `typeof === 'object'` and non-null, so the old
		//        gate accepted them, yet they carry no config fields and
		//        destructure to all-undefined → the same insecure resolver.
		//        This is the GHSA-8hr7-r645-pc6w bypass of the m4wx patch.
		//    `isPlainConfigObject` accepts ONLY `Object.prototype`- or
		//    null-prototyped objects, so the documented escape hatch (a truthy
		//    `nesting` + an explicit `require` config object, even `{}` or
		//    `Object.create(null)`) still works — a plain object counts as the
		//    developer's deliberate acknowledgment of the tradeoff — while every
		//    non-config shape above is rejected. A custom `Resolver` instance is
		//    also accepted (the `customResolver` path below bypasses
		//    `makeResolverFromLegacyOptions` entirely, so NESTING_OVERRIDE is
