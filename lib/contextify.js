	...
	object: function(object, traps) {
		let proxy = new Proxy(object, Object.assign({
			...
			set: (target, key, value, receiver) => {
				...
				try {
					return Reflect.set(target, key, Contextify.value(value), target);
				} catch (e) {
					throw Decontextify.value(e);
				}
			},
	...
	object: function(object, traps) {
		let proxy = new Proxy(object, Object.assign({
			...
			set: (target, key, value, receiver) => {
				...
				try {
					return Reflect.set(target, key, Decontextify.value(value), target);
				} catch (e) {
					throw Contextify.value(e);
				}
			},
	...
	value: function(value) {
		...
		switch (typeof value) {
			case 'object':
				if (value === null) {
					return null;
				} else if (value instanceof host.Number)         { return Number(value);
				} else if (value instanceof host.String)         { return String(value);
				} else if (value instanceof host.Boolean)        { return Boolean(value);
				...
				} else if (value instanceof host.Buffer)         { return new Buffer(value);
				} else {
