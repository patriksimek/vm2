...
// NOTE: there is deliberately no `global.Proxy = ...` here. `Proxy` is removed
// from the sandbox entirely by the `Object.defineProperties(global, ...)` block
// near the top of this file, which installs it as a SEALED `undefined`
// (`writable: false, configurable: false`). A plain assignment to that slot is
// a strict-mode write to a non-writable property: V8 silently ignores it on the
// context's global proxy, so the previous `global.Proxy = proxiedProxy;` line
// had been dead since the sandbox-global `Proxy` removal landed, while other
// engines (JSC/Bun) correctly throw and abort sandbox setup. `proxyHandler` and
// the handler-sanitising chain below it stay reachable through the `revocable`
// override above, which hardens the real Proxy constructor object in case a
// reference to it ever leaks. If `Proxy` is ever reintroduced to the sandbox,
// install it with `localReflectDefineProperty` and check the return value —
// never with a bare assignment.
