...
if (parseInt(version[0]) < 6) throw new Error(`vm2 requires Node.js version 6 or newer (current version: ${process.versions.node}).`);
	...
	constructor(options = {}) {
		...
		if (this.options.sandbox) {
			...
			for (let name in this.options.sandbox) {
				this._internal.contextify(this.options.sandbox[name], {global: name});
			}
		}
	}
...
class NodeVM extends EventEmitter {
	...
	constructor(options = {}) {
		...
		let closure = vm.runInContext(`(function (vm, host, contextify, decontextify) { ${sb} \n})`, this._context, {
			filename: "sandbox.js",
			displayErrors: false
		});
