...
| 22 | [Promise Executor Unhandled Rejection — Host Process DoS](attacks/host-resources.md#attack-category-22-promise-executor-unhandled-rejection--host-process-dos) | [Host Resources](attacks/host-resources.md) | compound | GHSA-hw58-p9xv-2mjh, GHSA-gjq8-xm47-88rc |
...
Write in the present tense. An entry describes a closed hole and the structure that keeps it closed; the fix is the Mitigation section, so retrospective markers announcing that a hole is fixed, and phrasings such as "historically" or "was dangerous", do not appear. Where the pre-fix behaviour genuinely has to be stated, scope it explicitly — "Before GHSA-xxxx, the trap did Y. The trap now does Z."
