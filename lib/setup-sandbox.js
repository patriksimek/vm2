...
Object.defineProperties(global, {
	...
	// SEALED (`writable: false, configurable: false` — the defaults for an
	// omitted attribute) so sandbox code cannot swap these intrinsics out.
	// `Error` in particular backs the `prepareStackTrace` defence (issue #467).
	// `Proxy` is removed from the sandbox outright; see the note next to the
	// `proxyHandler` definition before reintroducing it.
	Error: { value: LocalError, writable: false, configurable: false },
	Promise: { value: localPromise, writable: false, configurable: false },
	Proxy: { value: undefined, writable: false, configurable: false },
});
 ...
 * export. Mirrors the existing `WebAssembly.JSTag` removal (GHSA-9qj6-qjgg-37qq)
 * in spirit.
 *
...
// This line looks dead on a current runtime. It is not, and it must not be
// removed again without testing the FULL supported Node range.
//
// The `Object.defineProperties(global, ...)` block near the top of this file
// installs `Proxy` as a sealed `undefined` (`writable: false` by omission).
// Whether that seal actually takes on the vm context's global proxy is
// version-dependent:
//
//   * Modern V8  — the slot really is non-writable, so this assignment is a
//                  silent no-op and the sandbox has no `Proxy` at all.
//   * Node 8's   — the slot stays WRITABLE, so this assignment is live and is
//     older V8     what gives that sandbox its handler-sanitising `Proxy`.
//
// Deleting it therefore changed behaviour only on the oldest supported Node,
// where `new Proxy(...)` inside the sandbox began throwing. Not a security
// regression (removing `Proxy` is strictly more restrictive), but a real
// compatibility break on a runtime `engines` still admits.
//
// JavaScriptCore (Bun) implements the strict-mode write correctly and THROWS
// on the sealed slot, which would abort sandbox setup — so the write is guarded
// rather than bare. `localReflectDefineProperty` returns false instead of
// throwing when the slot is sealed, which is exactly the behaviour we want on
// every engine: apply it where the slot is writable, ignore it where it is not.
// Only `value` is specified: for an EXISTING property the omitted attributes
// are left as they are, so this changes nothing but the value and needs only
// `writable: true` to succeed — the same requirement the original assignment
// had. Specifying `configurable: true` here would additionally fail wherever
// the slot is non-configurable, which is precisely the case on Node 8.
localReflectDefineProperty(global, 'Proxy', { __proto__: null, value: proxiedProxy });
