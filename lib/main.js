...
const assertESM = () => {
	if (!vm.SourceTextModule) throw new Error(`To enable esm support in vm2, run node with --experimental-vm-modules flag.`);
};
...
class VMScript {
	...
	wrap(prefix, suffix) {
		if (this._wrapped || this.isESM) return this;
	...
	/**
	 * Returns true if the script is ESM.
	 */

	get isESM() {
		return /\.mjs$/.test(this.filename);
	}
}
...
class NodeVM extends EventEmitter {
	...
	run(code, filename) {
		...
		if (script.isESM) {
			...
			const esm = new vm.SourceTextModule(script.code, {
				context: this._context,
				importModuleDynamically(specifier, referencingModule) {
					throw new Error('TODO');
				}
			});
			return esm.link((specifier, referencingModule) => {
				throw new Error('TODO');
			})
			.then(() => esm.instantiate())
			.then(() => esm.evaluate({
				timeout: this.options.timeout
			}))
			.then(result => {
				return this._internal.Decontextify.value(result.result);
			})
			.catch(err => {
				throw this._internal.Decontextify.value(err);
			});
		} else {
