...
function createBridge(otherInit, registerProxy) {
	...
	function isRelevantPromisePeelFn(fn) {
		return isHostPromiseThen(fn) || isHostPromiseCatch(fn) ||
			isHostFunctionCall(fn) || isHostFunctionApply(fn);
	}
	function normalizeHostPromiseCallbacks(object, context, args) {
		...
		while (true) {
			if (guard++ >= MAX_PROMISE_PEEL) {
				...
				// confirmed the callbacks were wrapped, so deny rather than let a
				// raw host rejection through. Legitimate code never stacks
				// call/apply anywhere near this deep.
				throw new VMError(OPNA);
			}
			if (isHostPromiseThen(curFn)) {
				wrapPromiseSlot(container, base, false);    // onFulfilled
				wrapPromiseSlot(container, base + 1, true);  // onRejected
				return args;
			}
			if (isHostPromiseCatch(curFn)) {
				wrapPromiseSlot(container, base, true);      // onRejected
				return args;
			}
			if (isHostFunctionCall(curFn)) {
				...
				let underlying;
				try { underlying = otherFromThis(curThis); } catch (e) { return args; }
				if (!isRelevantPromisePeelFn(underlying)) return args;
			...
			if (isHostFunctionApply(curFn)) {
				...
				if (freshInner === null) return args; // unusable inner -> leave call untouched
			...
			// indirection primitive — nothing for this defense to do.
			return args;
		}
	}
	...
	class BaseHandler extends SafeBase {
		...
		apply(target, context, args) {
			...
			args = normalizeHostPromiseCallbacks(object, context, args);
			let ret; // @other(unsafe)
