...
function makeResolverFromLegacyOptions(options, override, compiler) {
	...
	// matches the realpath of candidate filenames. If realpath fails (e.g. root
	// doesn't exist yet, or fs has no realpath), fall back to the lexical resolve.
	// GHSA-cp6g-6699-wx9c.
	const checkedRootPaths = rootPaths ? (Array.isArray(rootPaths) ? rootPaths : [rootPaths]).map(f => {
		const resolved = fsOpt.resolve(f);
		try { return fsOpt.realpath(resolved); } catch (e) { return resolved; }
	}) : undefined;
