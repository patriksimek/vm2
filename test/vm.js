...
describe('VM', () => {
	...
	it.cond('WebAssembly.JSTag escape via wasm exception handling', typeof WebAssembly !== 'undefined' && WebAssembly.JSTag !== undefined, () => {
		const vm2 = new VM();
		// WebAssembly.JSTag allows wasm try_table/catch to intercept JS exceptions,
		// bypassing the transformer's catch block instrumentation entirely.
		// The caught host error is returned as an externref without sanitization.
		assert.throws(() => vm2.run(`
	...
	it.cond('WebAssembly.JSTag is not accessible in sandbox', typeof WebAssembly !== 'undefined' && WebAssembly.JSTag !== undefined, () => {
		const vm2 = new VM();
