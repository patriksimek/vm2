...
function createBridge(otherInit, registerProxy) {
	...
	class BaseHandler extends SafeBase {
		...
		get(target, key, receiver) {
			...
			let ret; // @other(unsafe)
			try {
				receiver = otherFromThis(receiver);
				ret = otherReflectGet(object, key, receiver);
			} catch (e) { // @other(unsafe)
				throw thisFromOtherForThrow(e);
			}
		...
		set(target, key, value, receiver) {
			...
			try {
				...
				receiver = otherFromThis(receiver);
				return otherReflectSet(object, key, value, receiver) === true;
			} catch (e) { // @other(unsafe)
				throw thisFromOtherForThrow(e);
			}
		}
		...
		construct(target, args, newTarget) {
			...
			try {
				args = otherFromThisArguments(args);
				newTarget = otherFromThis(newTarget);
				ret = otherReflectConstruct(object, args, newTarget);
			} catch (e) { // @other(unsafe)
				throw thisFromOtherForThrow(e);
			}
