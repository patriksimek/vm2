...
Bridge proxy `ownKeys`/`has`/`getOwnPropertyDescriptor` traps filter dangerous symbols. Sandbox overrides `getOwnPropertySymbols`, `ownKeys`, `getOwnPropertyDescriptors`, `assign` to filter or replace dangerous symbols with sandbox-local equivalents. The filtering uses `Reflect.defineProperty` with null-prototype descriptors to build result arrays, avoiding `Array.prototype` monkey-patching.
