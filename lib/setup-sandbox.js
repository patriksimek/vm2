 ...
 * does NOT harden the species channel itself. `Promise.prototype.finally` is still
 * species-sensitive, and a sandbox-realm Promise reached through it still resolves
 * its SpeciesConstructor via `resetPromiseSpecies` on the sandbox `then`/`catch`
 * overrides. Any future intrinsic that hands the sandbox a Promise whose
 * `[[Prototype]]` is host-realm re-opens the same flow and needs either its own
 * removal here or a species-side defense — which this block deliberately does not
 * attempt.
 */
if (typeof WebAssembly !== 'undefined') {
	...
	// The sandbox-side `globalPromise.prototype.then/catch` overrides and
