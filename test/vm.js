...
describe('VM', () => {
	...
	it('prototype manipulation sandbox escape', () => {
		// Prototype manipulation allows climbing from sandbox context to host context
		// If any method to modify prototypes is allowed, an attacker can:
		// 1. Get a host object (e.g., Buffer)
		// 2. Set a sandbox object's prototype to the host object's prototype
		// 3. Access the host Function constructor via the prototype chain
		// 4. Execute arbitrary code with full Node.js privileges
		//
		// All three prototype manipulation methods must be blocked:
		// - Object.setPrototypeOf
		// - obj.__proto__ = value (setter)
		// - Reflect.setPrototypeOf

		// Test 1: Object.setPrototypeOf must be blocked
		let vm2 = new VM();
		assert.throws(() => vm2.run(`
			const attack = {};
			Object.setPrototypeOf(attack, Object.getPrototypeOf(Buffer));
			// If this succeeds, attacker could do:
		...
		// Test 2: __proto__ setter must be blocked
		vm2 = new VM();
		assert.throws(() => vm2.run(`
			const attack = {};
			attack.__proto__ = Object.getPrototypeOf(Buffer);
			// If this succeeds, attacker could do:
			// attack.constructor.constructor('return process')()
		`), err => {
			assert.ok(err instanceof Error);
		...
		// Test 3: Reflect.setPrototypeOf must be blocked
		vm2 = new VM();
		assert.throws(() => vm2.run(`
			const attack = {};
			Reflect.setPrototypeOf(attack, Object.getPrototypeOf(Buffer));
			// If this succeeds, attacker could do:
			// attack.constructor.constructor('return process')()
		`), err => {
			assert.ok(err instanceof Error);
		...
		// Test 4: Verify the complete attack chain is blocked
		vm2 = new VM();
		assert.throws(() => vm2.run(`
			const attack = {};
			// Try each method in sequence (all should be blocked)
			try {
				Object.setPrototypeOf(attack, Object.getPrototypeOf(Buffer));
			} catch(e1) {
				try {
			...
			// If any succeeded, try the exploit
			attack.constructor.constructor('return process')();
		`), /Operation not allowed on contextified object|process is not defined/, '#4 complete attack chain');

		// Test 5: Verify safe prototype operations still work
		vm2 = new VM();
		assert.strictEqual(vm2.run('Object.getPrototypeOf({}) === Object.prototype'), true, '#5 getPrototypeOf works');
		assert.strictEqual(vm2.run('({}).__proto__ === Object.prototype'), true, '#6 __proto__ getter works');
		assert.strictEqual(vm2.run('({}) instanceof Object'), true, '#7 instanceof works');
		assert.strictEqual(vm2.run('Object.create(Array.prototype) instanceof Array'), true, '#8 Object.create works');
	});
