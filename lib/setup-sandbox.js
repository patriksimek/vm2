...
// The three engines disagree about this one write, so it is wrapped rather
// than written bare. Measured, not assumed:
//
//   Node >= 10  `Proxy` is a sealed `undefined` (writable: false). The write is
//               a silent no-op and the sandbox has no `Proxy` at all.
//   Node 8      the slot is writable AND configurable, and the old V8 global
//               proxy honours [[Set]] but silently ignores
//               [[DefineOwnProperty]] — `Reflect.defineProperty` returns true
//               while storing nothing. Only a plain assignment works, and it is
//               what gives that sandbox its handler-sanitising `Proxy`.
...
//
// So: a plain assignment (the only form Node 8 respects) inside a try/catch
// (because JSC throws). Swallowing is safe — the failure outcome is "no `Proxy`
// in the sandbox", which is strictly more restrictive, never less.
//
// Do not "simplify" this to Reflect.defineProperty: it reports success on
// Node 8 and does nothing, which is how this was broken once already.
try {
