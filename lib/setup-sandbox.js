...
function makeFunction(inputArgs, isAsync, isGenerator, newTarget) {
	...
	const func = localEval(code);
	if (newTarget) {
		const { prototype } = newTarget;
		if (typeof prototype === 'object' || typeof prototype === 'function') {
			localReflectSetPrototypeOf(func, prototype);
		}
	}
	return func;
}
...
const FunctionHandler = {
	...
	construct(target, args, newTarget) {
		return makeFunction(args, this.isAsync, this.isGenerator, newTarget);
	}
};
