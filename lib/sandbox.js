...
return ((vm, host) => {
	...
	const EXTENSIONS = {
		...
		[".node"](module, filename) {
			...
			try {
				module.exports = Contextify.value(host.require(filename));
			} catch (e) {
				throw Contextify.value(e);
			}
		},
	...
	const _requireBuiltin = function(modulename) {
		...
		if (modulename === 'util') {
			return Contextify.object(host.require('util'), {
		...
		return Contextify.object(host.require(modulename), {
			set: (target, key) => false
		});
	};
	...
	const _prepareRequire = function(current_dirname) {
		const _require = function(modulename) {
			if (vm.options.nesting && modulename === 'vm2') return {VM: Contextify.value(host.VM), NodeVM: Contextify.value(host.NodeVM)};
			...
			if (vm.options.require.mock && vm.options.require.mock[modulename]) {
				return Contextify.value(vm.options.require.mock[modulename]);
			}
	...
	global.process = {
		...
		versions: Contextify.value(host.process.versions),
		arch: host.process.arch,
		platform: host.process.platform,
		env: {},
		pid: host.process.pid,
		features: Contextify.value(host.process.features),
		...
		listeners(name) {
			return Contextify.value(host.process.listeners(name));
		},
	...
	if (vm.options.console === 'inherit') {
		global.console = Contextify.object(host.console, {
			set: (target, key) => false
		});
