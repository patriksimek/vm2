...
**Uses**: [Category 7](#attack-category-7-promise-and-async-exploitation)
...
Restores **[Defense Invariant #14](#defense-invariants)**. Structural fix in `lib/setup-sandbox.js`, entirely gated to `allowAsync: false` so the battle-tested `allowAsync: true` Promise shape — and every prior advisory defense built on it — is untouched:
...
- **WebAssembly JSPI / `WebAssembly.Suspending`** — produce cross-realm promises and are neutralized separately at bootstrap ([Category 33](#attack-category-33-webassembly-jspi-cross-realm-promise-prototype)); not an `allowAsync` scheduling vector once removed.
