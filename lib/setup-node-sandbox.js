...
function requireImpl(mod, id, direct) {
	...
	if (localStringPrototypeStartsWith(filename, 'node:')) {
		...
		// SECURITY (GHSA-6w8r-xxw2-g3hx): a stripped `node:` specifier must not
		// re-enter the `node:` namespace. `require('node:node:sqlite')` otherwise
		// sliced to `node:sqlite`, which matched a `node:`-prefixed builtin-map key
		// and gave a hidden second spelling for any node:-only builtin. Node itself
		// rejects repeated `node:` prefixes; do the same.
		...
		if (!nmod) {
			// node:-only builtins (e.g. `node:sqlite`) are registered under their
			// full `node:`-prefixed name; fall back to it when the bare id misses so
			// the canonical `require('node:sqlite')` spelling resolves (previously
			// only the buggy double-prefix form did).
			nmod = loadBuiltinModule(id) || loadBuiltinModule(filename);
