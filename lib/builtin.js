...
function defaultBuiltinLoaderUtil(vm) {
	...
	// SECURITY (GHSA-r273-hxvj-fxhp): build the exposed copy from the vetted
	// allowlist in `sanitizeUtilModule` instead of `Object.assign({}, util)`, so
	// no unreviewed host `util` member reaches the sandbox. `inherits` is then
	// overwritten with vm2's realm-safe replacement.
	const copy = sanitizeUtilModule(util);
...
const DANGEROUS_BUILTINS = new Set([
	...
	'worker_threads',
	'cluster',
	// SECURITY (GHSA-pq68-rvw4-xp4r): `child_process` is the most direct
	// host-process spawner — `execSync`/`exec`/`spawn`/`execFile`/`fork` run
	// arbitrary host commands, a full sandbox escape to host RCE. It has no
	// containable safe subset (unlike the member-neutralized crypto/tls/http
	// families), so like `cluster`/`worker_threads` it must never reach the
	// sandbox through the default host-passthrough loader — neither via `'*'`
	// wildcard expansion nor an explicit `builtin: ['child_process']` request.
	// Embedders who genuinely need a controlled child-process facade can still
	// register a sandbox-safe wrapper under the same name via `require.mock`.
	'child_process',
	'vm',
	'repl',
	'inspector',
...
// once again coherent. Both branches of `makeBuiltinsFromLegacyOptions`
// iterate this filtered list, so an explicit `builtin: ['_http_client']` entry
// is dropped as well; only the lower-level `makeBuiltins([...])` and
// `mock`/`override` registrations (via `addDefaultBuiltin`) can still name an
// internal sibling.
...
function sanitizeCryptoModule(mod) {
	...
	// SECURITY (GHSA-x3v6-43hc-82mc): crypto.setFips(bool) flips the FIPS mode of
	// the ENTIRE host process; a readonly wrapper stops property replacement but
	// not the side effect of a forwarded host call, so sandbox code could mutate
	// process-wide crypto configuration observed by trusted host code afterwards.
	// This is the same process-wide-mutator class already neutralized for
	// crypto.setEngine and tls.setDefaultCACertificates (GHSA-98xx-8mx4-x7cm).
	// `setEngine` and `setFips` are the only `set*` members crypto exposes, so
	// neutralizing both closes the class. getFips() (read-only) is untouched.
	// The deprecated `crypto.fips` accessor (DEP0093, setter backed by setFips)
	// never reaches the copy: it is non-enumerable, so Object.assign skips it,
	// and the read-only wrap refuses the write regardless.
	if (typeof copy.setFips === 'function') {
		copy.setFips = function setFips() {
			throw new Error('crypto.setFips is disabled in vm2 sandboxes: it changes the FIPS mode of the entire host process, mutating process-wide crypto configuration (GHSA-x3v6-43hc-82mc).');
		};
	}
	return copy;
}
...
// SECURITY (GHSA-r273-hxvj-fxhp): `util` was exposed as `Object.assign({}, util)`
// -- a wholesale copy-everything-forward of EVERY host `util` member. That is a
// structural hole, not a single leaky function: every future host `util` member
// auto-enters the sandbox the day Node ships it, with zero review. On Node
// >= 22.9 the first such member to matter is `util.getCallSites()`, which returns
// the host process call stack as plain data -- absolute file paths (vm2's own
// `lib/`, the embedder entrypoint), `node:internal` frames, function names and
// line numbers. It is produced host-side and never crosses the sandbox-realm
// Error stack formatter, so the GHSA-v27g host-frame redaction (which only
// rewrites the sandbox stack) does not cover it. `util.getCallSite` (the Node 22
// singular spelling) is the same class.
//
// POLICY CHOICE -- allowlist, not denylist. A denylist ("drop getCallSites")
// leaves the copy-everything-forward CLASS open: it re-opens the moment Node
// adds another host-introspection helper (a future stack/heap/host-state getter),
// exactly as `getCallSites` slipped in after every earlier vm2 release. An
// allowlist is forward-safe: a member Node adds tomorrow does NOT auto-enter the
// sandbox -- admitting it becomes a conscious, reviewed edit to this list. The
// trade-off is maintenance (a genuinely new *legitimate* member must be added
// here) and the risk of omitting a member some user relies on; we accept that in
// exchange for closing the class, and enumerate the full documented surface
// below to keep the omission risk low. Members are copied by reference, so
// attached props survive intact (`inspect.custom`, `promisify.custom`, the whole
// `types` namespace). Presence-gating (`name in mod`) makes the list safe across
// Node 8..26 where the surface varies -- absent members are simply skipped.
//
// Deliberately NOT copied (host-introspection / host-process-mutation / private
// internals -- the members whose removal is the point of this fix):
//   getCallSites, getCallSite   host call stack -> host paths (the vuln)
//   setTraceSigInt              installs a process-wide host SIGINT trace hook
//   _errnoException,            private error-construction internals
//     _exceptionWithHostPort
//   convertProcessSignalToExitCode  undocumented Node 26 internal helper
//   inherits                    replaced with vm2's realm-safe version by the caller
const SAFE_UTIL_MEMBERS = [
	// Formatting / inspection.
	'format', 'formatWithOptions', 'inspect', 'log',
	// Callback <-> promise adapters.
	'promisify', 'callbackify',
	// Deprecation / debug helpers.
	'deprecate', 'debuglog', 'debug',
	// Deep comparison.
	'isDeepStrictEqual',
	// Type-check namespace + legacy deprecated is* predicates.
	'types',
	...
	// Encoding / text.
	'TextEncoder', 'TextDecoder', 'stripVTControlCharacters', 'styleText',
	'toUSVString', 'MIMEType', 'MIMEParams',
	// Errno lookup tables (static maps, not host state).
	'getSystemErrorName', 'getSystemErrorMap', 'getSystemErrorMessage',
	// Argument / env parsing, misc data helpers.
	'parseArgs', 'parseEnv', 'diff', 'aborted',
	'transferableAbortController', 'transferableAbortSignal',
	// Deprecated shallow-copy + legacy Node 8 I/O helpers.
	'_extend', 'error', 'print', 'puts'
];

function sanitizeUtilModule(mod) {
	const copy = {};
	for (let i = 0; i < SAFE_UTIL_MEMBERS.length; i++) {
		const name = SAFE_UTIL_MEMBERS[i];
		// Presence-gate so the list is safe across Node majors (members vary);
		// only forward what this host actually exposes.
		if (name in mod) copy[name] = mod[name];
	}
	return copy;
}
...
const BUILTIN_MEMBER_SANITIZERS = {
	...
	// SECURITY (GHSA-r273-hxvj-fxhp): route `util` and its deprecated alias `sys`
	// through the same audited member-neutralization chokepoint as crypto/tls/etc.
	// `util` normally loads via its SPECIAL_MODULES entry (which calls
	// `sanitizeUtilModule` directly for the custom-`inherits` path); `sys` reaches
	// the sandbox through the generic readonly loader, so it must be sanitized here.
	util: sanitizeUtilModule,
	sys: sanitizeUtilModule
};
