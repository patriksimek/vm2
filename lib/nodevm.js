...
class NodeVM extends VM {
	...
	constructor(options = {}) {
		...
		// `nesting: true` injects a NESTING_OVERRIDE builtin that exposes `vm2` to
		// the sandbox. When `requireOpts` is also falsy, `makeResolverFromLegacyOptions`
		// hits its `if (!options)` branch and builds a resolver whose ONLY builtin
		// is `vm2` — a pure escape primitive (sandbox does `require('vm2')`, builds
		// an inner NodeVM with attacker-chosen `require` config, reaches
		// `child_process` for host RCE).
		//
		// The check must run *after* destructuring and against `requireOpts`, not
		// against the raw `options.require`, because the destructuring default
		// (`require: requireOpts = false`) collapses every falsy/omitted path to
		// the same insecure resolver:
		//   - `require: false`           → requireOpts = false   (GHSA-8hg8 PoC)
		//   - `require` omitted          → requireOpts = false   (GHSA-m4wx PoC)
		//   - `require: undefined`       → requireOpts = false   (default kicks in)
		//   - `require: null` / `0` / '' → requireOpts is null/0/''
		// All of them hit `if (!options)` in makeResolverFromLegacyOptions and
		// produce the NESTING_OVERRIDE-only resolver. Use `!requireOpts` to reject
		// every shape that yields that resolver; the documented escape hatch
		// (`nesting: true` + an explicit `require` config object, even `{}`)
		// continues to work — the developer's intent is then visible.
		if (nesting === true && !requireOpts) {
			throw new VMError(
				'NodeVM `nesting: true` requires an explicit `require` config. '
				+ '`nesting: true` is an escape hatch that exposes `vm2` to the '
				+ 'sandbox (sandbox code can `require(\'vm2\')` and construct nested '
				+ 'VMs unconstrained by the outer config). With `require` falsy or '
				+ 'omitted, the resolver exposes ONLY `vm2` — a pure escape primitive. '
				+ 'To deny all requires, remove `nesting: true`. To allow nested VMs, '
				+ 'pass an explicit `require` config (e.g. `require: { builtin: [] }`) '
				+ 'so the tradeoff is visible. See README "`nesting: true` is an escape '
				+ 'hatch". Context: GHSA-m4wx-m65x-ghrr (supersedes GHSA-8hg8-63c5-gwmx).'
			);
		}
